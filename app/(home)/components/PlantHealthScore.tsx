...
const INITIAL_MODULES: ModuleData[] = [
  {
    ...
    indicators: [
      ...
      {
        label: "Overdue SCE Tests",
        value: "3 Overdue",
        statusColor: "text-red-600 dark:text-red-400",
      },
      {
        label: "SCE Performance Standard Status",
        value: "Needs Review",
  ...
  },
  {
    ...
    indicators: [
      {
        label: "Risk Matrix Evaluation Status",
        value: "Current",
        statusColor: "text-emerald-600 dark:text-emerald-400",
      },
      {
        label: "High-Risk Asset Count",
        value: "12 High Risk",
  ...
  },
  {
    ...
    indicators: [
      ...
      {
        label: "Deferred Anomalies",
        value: "8 Deferred",
        statusColor: "text-yellow-600 dark:text-yellow-400",
      },
      {
        label: "Completed Anomalies",
        value: "14 Closed",
        statusColor: "text-emerald-600 dark:text-emerald-400",
      },
      {
        label: "High-Priority Anomaly Exposure",
        value: "High",
        statusColor: "text-orange-600 dark:text-orange-400",
      },
    ],
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-2">
              {INITIAL_MODULES.map((m) => (
                <div
                  key={m.id}
                  className="flex justify-between items-center text-xs p-2 rounded-lg bg-slate-50 dark:bg-slate-800/50"
                >
...
function BreakdownAccordionItem({
  ...
  return (
    <div
      className={`transition-all duration-500 rounded-xl overflow-hidden border ${isHighlighted
            ...
            <div className="space-y-3">
              {indicators.map((ind, idx) => (
                ...
                >
                  ...
                  <span
                    className={`font-semibold leading-none ${ind.statusColor || "text-slate-900 dark:text-slate-200"
                      }`}
                  >
                    {ind.value}
                  </span>
...
function ScoreBreakdownDrawer({
  ...
  return (
    <>
      <div
        aria-hidden
        className={`fixed inset-0 z-[80] bg-slate-900/40 dark:bg-black/70 backdrop-blur-sm transition-opacity duration-300 ${open ? "opacity-100" : "opacity-0 pointer-events-none"
          }`}
        onClick={onClose}
      />
      ...
      >
        <div className="flex items-center justify-between px-6 py-6 shrink-0">
          <div>
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5 uppercase tracking-wide font-medium">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-10">
          <div className="flex items-center gap-6 p-5 rounded-2xl bg-slate-50/50 dark:bg-slate-900/60 border border-slate-200/50 dark:border-white/10 shadow-lg">
            ...
            <div className="space-y-1">
              ...
              <span
                className="text-[11px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full text-white shadow-sm"
            ...
            <div className="space-y-2.5">
              {modules.map((section) => (
                <BreakdownAccordionItem
                  ...
                  isHighlighted={
                    showVisualGlow && highlightedModuleId === section.id
                  }
                />
              ))}
            </div>
        ...
        <div className="p-6 mt-auto space-y-3">
          ...
          <div className="p-4 rounded-xl bg-blue-50/30 dark:bg-blue-900/10 border border-blue-200/20 flex items-start gap-3">
            ...
            <p className="text-[11px] text-slate-500 dark:text-slate-400 leading-relaxed font-medium">
              This score represents the weighted average of real-time integrity
              and performance indicators. "Impacting" areas are those performing
              below the health threshold of {HEALTH_THRESHOLD}%.
            </p>
          </div>
        </div>
      </div>
...
function PlantHealthScoreContent() {
  ...
  useEffect(() => {
    if (
      highlightParam === "asset_performance" ||
          ...
          <div className="flex flex-col items-center text-center sm:text-left sm:flex-row gap-10 w-full lg:w-auto">
            ...
            >
              <svg className="w-full h-full -rotate-90">
                ...
                <circle
                  ...
                  strokeDashoffset={
                    (1 - currentDisplayScore / 100) * circumference
                  }
                  strokeLinecap="round"
            ...
            <div className="flex flex-col items-center sm:items-start space-y-5">
              ...
              <p className="text-xs text-slate-500 dark:text-slate-400 max-w-[240px] leading-relaxed">
                The score integrates asset integrity, safety standards, and
                management compliance data.
              </p>
          ...
          <div className="w-full flex-1 space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              {SCORE_RANGES.map((range) => {
                const isCurrent =
                  finalScoreValue >= range.min && finalScoreValue <= range.max;

                return (
                  <div
                    key={range.label}
                    className={`flex items-center justify-between rounded-xl px-5 py-4 transition-all duration-300 border ${isCurrent
                  ...
                  >
                    ...
                    <span
                      className={`text-[12px] font-mono font-medium ${isCurrent
