        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 shadow-md shadow-slate-200/50 dark:shadow-none overflow-hidden min-h-[calc(100vh-140px)] flex flex-col">
                ...
                <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex justify-between items-center">
                    ...
                    <button className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-indigo-600 bg-white dark:bg-slate-900 border border-indigo-200 dark:border-indigo-800 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                        <Upload size={16} />
                        Bulk Upload
                    </button>
                </div>
                ...
                {/* Toolbar (Filters) */}
                <div className="p-4 border-b border-slate-200 dark:border-slate-800 flex flex-wrap gap-4 items-center bg-slate-50/50 dark:bg-slate-900/20">
                    ...
                    <div className="relative flex-1 min-w-[250px]">
                        ...
                        <input
                            ...
                            className="w-full pl-9 pr-4 py-2 text-sm border border-slate-200 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-950 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 placeholder:text-slate-400"
                        />
                    </div>
                    ...
                    <div className="flex gap-4 flex-wrap">
                        {['All Types', 'All Systems', 'All Packages', 'All Status', 'All Groups'].map((filter, idx) => (
                            <div key={idx} className="relative">
                                <select className="appearance-none pl-4 pr-10 py-2 text-sm border border-slate-200 dark:border-slate-800 rounded-lg bg-white dark:bg-slate-950 text-slate-700 dark:text-slate-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer min-w-[130px]">
                        ...
                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800 text-slate-700 dark:text-slate-300">
                            {equipments.map((item, idx) => (
                                <tr key={idx} className="hover:bg-slate-50 dark:hover:bg-slate-900/50 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-indigo-600 dark:text-indigo-400 font-medium min-w-[250px]">{item.name}</td>
                                    <td className="px-6 py-4">{item.type}</td>

                                    {/* Multiline cells (System/Package) */}
                                    <td className="px-6 py-4">
                                        {item.system.split('\n').map((line, i) => (
                                            <div key={i} className={i === 0 ? "text-indigo-600 dark:text-indigo-400 font-medium whitespace-nowrap" : "text-slate-400 text-xs mt-1"}>
                                                {line}
                                            </div>
                                        ))}
                                    </td>
                                    ...
                                    <td className="px-6 py-4 text-right">
                                        <div className="flex justify-end">
                                            <button className="p-1.5 text-indigo-500 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-md border border-slate-200 dark:border-slate-700 transition-colors">
                                                <Eye size={16} />
                                            </button>
                                        </div>
                                    </td>
                ...
                {/* Pagination Footer */}
                <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between text-sm text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-950 mt-auto">
                    <div>Showing 1 to 10 of 1000 equipment</div>
                    <div className="flex items-center gap-1">
                        <button className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 mr-1 disabled:opacity-50"><ChevronLeft size={16} /></button>
                        <button className="px-3 py-1 rounded bg-indigo-600 text-white font-medium">1</button>
                        <button className="px-3 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-indigo-600 dark:text-indigo-400 font-medium border border-slate-200 dark:border-slate-800">2</button>
                        <button className="px-3 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-indigo-600 dark:text-indigo-400 font-medium border border-slate-200 dark:border-slate-800">3</button>
                        <span className="px-2 font-medium">...</span>
                        <button className="px-3 py-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 text-indigo-600 dark:text-indigo-400 font-medium border border-slate-200 dark:border-slate-800">100</button>
                        <button className="p-1 rounded hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-200 dark:border-slate-800 ml-1"><ChevronRight size={16} /></button>
                    </div>
                </div>
